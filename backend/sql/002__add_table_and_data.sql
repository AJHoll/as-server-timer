...
COMMENT ON COLUMN timer_operation.operation_end IS 'Временная метка окончания операции';
COMMENT ON COLUMN timer_operation.operation_begin IS 'Временная метка начала операции';
COMMENT ON COLUMN timer.state IS 'Состояние таймера';
